...
impl Character {
    ...
    /// (External) then Plating (External / Contact) — and return what's left for the next
    /// layer. (The attack pipeline slips **hit-location chrome** in between this and
    /// [`apply_integrity`](Self::apply_integrity); DoTs go straight through.)
    ...
    /// Apply `remaining` damage straight to **Integrity** (no armor left to soak): the kill
