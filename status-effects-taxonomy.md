...
**DoT identity levers ◆:** (a) decay (none / self-consume / build-up); (b) bypass (true-damage vs HP-loss — bleed's signature is ignoring armor); (c) delivery (DD's "rider on a real hit" vs "payload, weak hit"); (d) resource (HP vs Stress/alt — in this game an alt-resource DoT would hit the **Resolve** pool (= Nerve × K), the morale/composure bar, with **Nerve** as its resist, mirroring DD's Stress 🔭); (e) tick timing (start = pre-emptive, end = unit acts first).
...
| Composure / will (spoof, intimidation, Stress resist) | the **Nerve** attribute 🔭 (genre's *Cool*; sizes **Resolve** = Nerve × K) |
...
- **Spoofing (a netrunner tool).** IFF is **code**, so a netrunner can **spoof** it — flip a unit's friend/foe reading: make your selective digital fire hit your **own**, slip an enemy past as "friendly," or blind your auto-targeting. The more your force trusts IFF, the more a spoof turns it inside out — the cyberware-liability theme applied to targeting. Unlike a worm (which melts **ICE** to crack the *system*), a spoof targets *agency* — it doctors what a unit perceives — so it resists off **Nerve** 🔭, the composure stat (see the spoof toolkit below): **ICE guards the system, Nerve guards the self**.
...
**Spoof toolkit (what an IFF spoof does) ◆.** A spoof corrupts a unit's IFF — a netrunner hack or the **Spoofer** worm (§7G); it's digital/Link-delivered and **code-only** (immune at zero Link, like a worm), but it resists off **Nerve** 🔭, *not* ICE. The principle: **ICE guards the system; Nerve guards the self.** A worm melts ICE to crack your data/surface; a spoof *edits your senses to hijack your behavior*, so it's your **composure (Nerve)** — your force of personality holding steady against a doctored read — that throws it off, not your firewall. Same delivery as a worm (digital, Link-gated), different **target** (agency, not system) and **resist** (Nerve, not ICE). **Machines have Nerve 0** → no composure → they are the **most spoof-credulous** chassis (Worm-vulnerable *and* spoof-credulous through their Link), the opposite of spoof-proof:
...
9. **Resist / counterplay** (the defense model, §7A): tier (Internal bypasses Barrier+Plating / Contact mitigated by Plating / External hits Barrier) · the **Body** attribute (Virus) · **ICE** (Worm/hacks — the *system* resist) · **Nerve** (spoof / intimidation / Stress — the *composure* resist; **ICE guards the system, Nerve guards the self**) · cleanse → Vaccinated/Antimalware · zero-Link (digital immunity) · stat-check self-cure.
...
| **Spoofer** | force an **IFF spoof** (friendly-fire / masquerade, §7F) — Link-delivered like the other worms, but the spoof itself **resists off Nerve, not ICE** 🔭 (composure, not firewall, throws it); can self-propagate |
...
The **answer half** of the digital realm. The offense in §7F (worms, hack-effects, spoofs, beams, netrunner hacks) needs dedicated counters, or going online is suicide, nobody builds Link, and the whole layer is wasted. These are the defenders — sitting on top of the baseline defenses (**ICE** vs worms/hacks, **Nerve** vs spoofs 🔭 — *ICE guards the system, Nerve guards the self*, **low/zero Link**, **cleanse → Antimalware/Vaccinated**).
...
| **Bulwark** (ICE-projector) | very high ICE, **extended to allies** | hacks, worm-catch, Drainware — raises the *system* resist roll (ICE); **spoofs resist off Nerve, not ICE**, so they slip past a pure ICE wall |
| **Signals officer** (counter-spoof) | **locks IFF** in range / reveals masqueraders & ghosts / reverses spoofs | the spoof menu (§7F) — the *active* answer on top of a unit's baseline **Nerve** composure 🔭 |
...
**Coverage check ◆:** every offense has an answer. **Bulwark** and **Patcher** are the load-bearing generalists (between them they blunt spread, DoT, and hacks); the **air-gapped blade** is the flesh-side catch-all; spoofs are answered by **Nerve** (the baseline composure resist 🔭) plus **Signals**, the *only* dedicated active spoof answer (a deliberate single point — a spoof-heavy enemy *forces* you to tech Signals or stack Nerve); **Jammer** is the only beam/area answer. Watch one thin spot: nothing hard-counters **Cascade** (mass hack-effect detonation) except running **lean chrome** — by design, since the answer to "too much chrome" is "less chrome."
...
|---|---|---|---|---|
| **Flesh** (organic) | biological body | **Virus**-vulnerable | low/zero (needs cyberware for Link) | the baseline; home of bioware + air-gapped builds; full **Nerve** (Resolve, composure) |
| **Augmented** (cyborg) | flesh + chrome | **both** — double-exposed | mid–high | capable-but-exposed middle; most versatile; full **Nerve** |
| **Machine** (drone / construct) | pure synthetic | **Virus-immune**; Worm-vulnerable *only if* Link > 0 | varies | flesh-immune, all-digital; bioware n/a; **Nerve 0** 🔭 — no Resolve (morale- & intimidation-proof) **but** no composure → the **most spoof-credulous** chassis when networked (do not mistake for spoof-proof) |

Chassis sets base stats, which contagions bite, the Link floor, **Nerve** (Resolve / composure — machines have none), and whether bioware/cyberware apply. **Drafting = pick a chassis (class); build the role in the shop.** The §7H "specialists" resolve as loadouts: antivirus / jammer / signals / quarantine / honeypot = **programs**; bulwark / air-gapped blade = **builds**.
...
13. **Spoof resolution (§7F):** *spoof menu drafted* (Flip-hostile / Masquerade / Scramble / Ghost); *physical-weapon question resolved* — the **Smartgun mod** (§7I) is the one IFF-guided (thus spoofable) gun upgrade; dumb weapons can't be spoofed. *Resist resolved (◆/🔭):* a spoof resists off **Nerve, not ICE** (composure guards the self), still Link-gated/code-only. Remaining: the **margin = hack-power − Nerve** numbers, and whether Stress/intimidation (the wider Nerve/Resolve morale layer) ship in this scope.
...
> **chassis** (Flesh / Augmented / Machine) · the four primary attributes **Body / Dexterity / Intellect / Nerve** (Body carries Integrity/HP, melee damage, **and** bio Internal-resist; **Nerve** carries the **Resolve** pool, morale, social presence, **and** composure — the spoof / intimidation / Stress resist) · **Integrity** · **Barrier**, **Plating** (+ material) · **physical Initiative** · **Link** (= digital Initiative) · **ICE** (digital Internal-resist) · **Resolve** (= Nerve × K) 🔭 · **weapon(s) + mods** · **augments** (cyberware / bioware) · **software** · a **movement profile** · a **targeting profile**. *(Heat — a thermal axis — was here; dropped for now.)*
...
**Hack resolution ◆** *(resolves §8 #5/#13).* **Margin = hack-power − target ICE** (the attacker's **icebreaker breaks** the ICE); success if **> 0**, with effect magnitude scaling on the margin. A success can: **disable** an implant, **deploy a worm**, or **trip the implant's hack-effect on its owner** (Seizure / Misfire / Shed / Overload / Lockout / Blind / Overdose, §7F). **Spoofing the unit's IFF is the exception ◆:** it's still Link-delivered (reach gated on Link > 0), but it resists off **Nerve, not ICE** 🔭 — **margin = hack-power − target Nerve** (composure, not firewall, throws the doctored read) — yielding **Flip-hostile** (attacks its own), **Masquerade** (enemy reads friendly), **Scramble** (can't target), or **Ghost** (untargetable by IFF effects). A **Nerve 0** target (machines) has no composure → spoofs land freely.
