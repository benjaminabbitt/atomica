...
- §7A Your **defense-penetration** model (Internal / Contact / External; armor materials; Immunity/Firewall)
...
- **Stochastic?** — does each tick *roll* (Poison, Disease — Con modifies the roll) or land a fixed number (Fire)?

Worked examples (the axes don't move together):
- **Fire / Burn** = Contact + DoT + **deterministic** → mitigated by **Plating** (armor).
- **Poison / Corruption** = Internal + DoT + **stochastic** → the **Internal resist** (Immunity if bio, Firewall if digital) modifies the roll.
...
**Each side is a *family*, not one effect ◆.** "Virus" and "Worm" are **keyword pools** (consistent with types ≠ statuses, §3.6): different strains/worms do different things — spread, disable an implant, DoT, strip Firewall, or **trip a target's hack-effects (a specific *named* one, or all at once)** (§7F) — sharing the per-side traits in the table. The spreading-contagion strain is the **flagship** of each. The Worm reaches a unit through its **Link** (the surface) by any of three vectors — **runner-directed, interaction, or ambient proximity** — all opposed by **Firewall** (see §7F).
...
| Resisted by | **Immunity** (bio Internal-resist) | **Firewall** (digital Internal-resist) + **anti-Worm specialists** (killer programs / hunters) |
...
- **Stochastic progression roll** each turn: **T1 tick** → **T2 build** (+1 stack) → **T3 spread**, higher-tier odds rising with stacks and pushed *down* by the relevant resist (Immunity / Firewall). The roll is the contagion's pulse.
...
2. **Two resists (decided), asymmetric.** Immunity (vs Virus) and Firewall (vs Worm) are **separate stats** — a brute can be Immune-tough but Firewall-soft, a netrunner the reverse (build identity). The counter-ecosystems differ: **Virus** → Immunity + cleanse (mostly innate); **Worm** → stackable Firewall guards **plus active anti-Worm specialists** (killer programs, hunter units). The digital side is the more *tooled* one.
...
| **Internal** | ✗ | ✗ | straight to Integrity (countered by **Immunity** / **Firewall**) | Warframe **Slash/Bleed** (True damage, ignores armor) ✓✓; ATO Bleed (ignores Block) ✓✓ |
...
Because the Internal tier has **two damage flavors** (§7E) — bio and digital — it splits into **two independent resists**:
- **Immunity** — vs **bio** Internal (Virus, organic toxins). The "immune system" stat; also the Virus growth-resist / self-cure (§7).
- **Firewall** — vs **digital** Internal (Worm, intrusions, malware). The security-rating stat; also the Worm catch-resist and roll-modifier (§7), and the gate on how exposed your interactions/connectivity are.

They're independent on purpose — a brute can be **Immune-tough, Firewall-soft**; a netrunner the reverse — which is real build diversity but a doubled resist economy (§8: keep two stats, or one umbrella "Constitution" with two channels?). Chrome (External/Contact) is still guarded by Barrier + Plating, untouched by either.
...
- **Equipment = weight + a digital bundle (the unified load model) ◆.** Every piece of gear carries **physical weight** (→ lowers **physical Initiative**) *and* a **(Link, Firewall, Hack-effect)** bundle (§7F): its Link **raises digital Initiative** but makes the unit louder/more exposed. So gearing up is one coherent trade — **slower body, faster/louder deck, better-defended, more hackable**; lean is the inverse. The whole **chrome-vs-flesh axis = how much equipment you run.**
...
- **Net (software) = Internal.** Bypasses chrome, hits the system directly: **viruses, intrusions, malware**. Guarded by **Firewall** (the digital Internal-resist, §7A). **Stochastic** — intrusions roll (§3.5).

**Two viruses (§7), two channels:** the bio **Virus** (spreads by hex-adjacency, resisted by **Immunity**, hits flesh) and the digital **Worm** (spreads by interaction, resisted by **Firewall**, hits systems). Position-counterplay vs action-counterplay.
...
| Internal defense (bio) | **Immunity** |
| Internal defense (digital) | **Firewall / Encryption** |
...
| **Firewall** | digital defense on the implant / unit — opposes hacks & worms | improvement |
...
So chrome is double-edged: more implants = more power (Link / Firewall / hacks) **and** more hackable surfaces whose hack-effects can be turned against you. Lean = fewer liabilities; loaded = stronger but more exploitable. The deleterious hack-effect should be **telegraphed** so buying chrome is an *informed* gamble (§8).
...
**Equipment taxonomy ◆.** A unit's **equipment** (the major purchasable) = **weapons** (§7I) + **augments** (cyberware / bioware, below) + **consumables** (grenades). Every piece carries **weight** (→ physical Initiative, §7C); cyberware additionally carries the **(Link, Firewall, Hack-effect)** digital bundle. Lean loadouts strike first; heavy ones hit harder but slower and more exposed.

**Augmentation — bioware vs cyberware ◆.** **Both branches enhance the physical body** — that axis is covered by the pair. The difference is the **digital surface**: cyberware drags a Link/Firewall/Hack bundle along (net capability *and* exposure), bioware adds none. It's the chrome-vs-flesh axis at the *build* level:

| | **Cyberware** (chrome) | **Bioware** (bio-augment) |
|---|---|---|
| Profile | physical benefit **+ a (Link, Firewall, Hack-effect) bundle** | physical benefit, **no Link** |
| Enhances | the **physical body** (stats, combat) — *plus* the digital realm (Link, hacks, beams, Firewall) | the **physical body** only (stats, regen, resilience) |
...
**Reject — the bioware liability ◆** (parallel to hack-effects). Just as a breached *implant* fires its hack-effect on its owner, an infected *bio-augment* can **reject** — malfunction deleteriously when the host catches a Virus: an adrenal graft floods (self-DoT), a muscle graft seizes (−Initiative), a regen-organ turns septic (−Immunity). So **all enhancement is double-edged** — cyber on the *hack* vector, bio on the *infect* vector. *(Design option: bioware could instead be simply "safer but digitally useless," with no Reject; Reject is the symmetry choice.)*

**Netrunner / hacker units** = the digital attackers. A hack **targets cyberware via its Link** (reachable only if Link > 0) and resolves **against its Firewall** (Firewall reduces/blocks). Success → disable the implant, deploy a Worm, or **trigger the implant's hack-effect on its owner**.
...
**Three attack vectors — all gated by Link, opposed by Firewall:**
...
**Spoof toolkit (what an IFF spoof does) ◆.** A spoof corrupts a unit's IFF — a netrunner hack or the **Spoofer** worm (§7G); resolves vs **Firewall**, Link-gated, **code-only**:
...
| **Leech** | **offensive** — drain a connected enemy's Firewall/Link in proximity; soften for your runners | — |

A unit's digital identity = the sum of its implants' **link-effects + Firewalls + hack-effects** (liabilities included). *Open (§8): which link-effects ship, and the deleterious-hack-effect severity that makes chrome a real-but-fair gamble.*
...
1. **Effect** (the payload): DoT · burst · heal/HoT · stat-mod (Initiative / atk / def / Firewall / Immunity / Link) · control (skip / lock) · defense-mod (grant or shred Barrier/Plating) · amplify/convert (vuln / resist) · spread/contagion · trigger-effect (detonate / trip a named hack-effect) · move/reposition.
...
9. **Resist / counterplay** (the defense model, §7A): tier (Internal bypasses Barrier+Plating / Contact mitigated by Plating / External hits Barrier) · **Immunity** (Virus) · **Firewall** (Worm/hacks) · cleanse → Vaccinated/Antimalware · zero-Link (digital immunity) · stat-check self-cure.
...
- **Spread targeting** → a contagion effect + a gating resist (Immunity / Firewall) + stochastic.
...
- **Strip / shred effect** → an *enabler*: it pairs with a follow-up (Firewall-strip opens the Worm; Plating-shred opens Contact damage).
...
|---|---|---|---|---|---|
| **Virus** (bio) | DoT + self-spread | per-turn → start-of-turn | cleanse-only → *Vaccinated*; intensity, **capped** | %max ramp; **stochastic** | Internal; **Immunity**, cleanse; **adjacency** |
| **Worm** (code) | DoT + self-spread (+ trip named hack-effect) | per-turn / on-interaction → start-of-turn | cleanse-only → *Antimalware*; intensity, **capped** | %max ramp; **stochastic** | Internal; **Firewall**, Link-gated, anti-Worm units; **interaction + proximity** |

**Worm strains (Family A, code side) ◆** — all share the Worm base (Internal · stochastic · Firewall-resisted · Link-gated · code-only; reaches via runner / interaction / proximity, §7F); they vary by *job*:
...
| **Drainware** | −Firewall / −Link over time — enabler that softens for follow-up |
| **Spoofer** | force an **IFF spoof** (friendly-fire / masquerade, §7F); can self-propagate |

*(Bio **Virus** strains below — the flesh-side mirror.)*

**Virus strains (Family A, bio side) ◆** — all share the Virus base (Internal · stochastic · Immunity-resisted · **adjacency spread** (positional, *not* Link) · infects flesh: organic + augmented, machines immune; cleanse → *Vaccinated*). They **mirror the worm strains in structure** but attack the **body**, not the chrome:
...
| **Poison** | DoT (festers) | per-turn → start-of-turn | cleanse-only or decrement; intensity, **capped** | flat or %; **stochastic** | Internal; **Immunity** (Internal resist); single (no spread) |
...
| Status | Effect | Trigger → Timing | Decay / Stacking | Mag / Behavior | Resist / Spread |
|---|---|---|---|---|---|
| **Crash** (stun) | skip next action | on-apply → immediate | one-shot or 1-turn; binary | N/A; deterministic | control-resist / Firewall (if digital); single |
...
| Status | Effect | Trigger → Timing | Decay / Stacking | Mag / Behavior | Resist / Spread |
|---|---|---|---|---|---|
| **Firewall-strip** | −Firewall (opens Worm) | on-apply → immediate | decrement or fixed; intensity | flat; deterministic | — ; single |
| **Immuno-suppress** | −Immunity (opens Virus) | on-apply → immediate | decrement or fixed; intensity | flat; deterministic | — ; single |
...
Either carries **any** status — a **Crash grenade** = the AoE stun the roster otherwise lacks, a Firewall-strip beam = soften a line for follow-up. **Friendly fire is on (decided):** a physical blast catches your own units; positioning is the mitigation (digital beams can spare allies via IFF — §7F). The **clustering tension** stands — grenades + Virus adjacency + Worm proximity all punish bunching, while Relay/Mesh buffs + front-line density reward it.
...
The **answer half** of the digital realm. The offense in §7F (worms, hack-effects, spoofs, beams, netrunner hacks) needs dedicated counters, or going online is suicide, nobody builds Link, and the whole layer is wasted. These are the defenders — sitting on top of the baseline defenses (**Firewall**, **low/zero Link**, **cleanse → Antimalware/Vaccinated**).
...
- **Out-tech (code).** Fight code with code — white-hat programs, hardened Firewall, signal control. Stays capable on the net but pays slots/tempo for defense. Archetypes: **antivirus / bulwark / signals**.
...
| **Bulwark** (Firewall-projector) | very high Firewall, **extended to allies** | hacks, worm-catch, spoofs, Drainware — raises the resist roll |
...
**EMP weapons — physical attacks on digital hardware ◆.** The physical realm's answer to the digital one. An EMP is a **physical** weapon (dumb, honest, unspoofable, hits its hexes) that **damages Link / antennae / cyberware directly** — and because it *fries hardware* rather than *hacking* it, it **bypasses Firewall** (digital defense doesn't apply to a physical pulse). Effects: **−Link** (fry the antenna → less net presence, digital Initiative, surface), **disable cyberware** (implants offline — a strong EMP can even trip their **hack-effects** on the way down, §7F), and **kill beams** (no antenna, no transmission).
...
|---|---|---|
| **Physical** weapon | guns / melee — vs armor (§7A) | **EMP** — vs hardening; *ignores Firewall* |
| **Digital** attack | worm / virus DoT (Internal, §7) | hacks / worms — vs Firewall, via Link |
...
5. **Netrunner attack resolution (§7F):** hack-power vs Firewall math, and what a successful hack can do (disable / deploy Worm / trip hack-effect).
6. **Worm catch math (§7):** base per-interaction odds, how Firewall lowers and Link raises them; confirm no same-action re-spread; do connected *attacks* transmit, or only connective actions?
...
- **Tick** — **[weak] Internal** damage to Integrity (Immunity / Firewall lowers the roll).
...
**Commit-time trap (Worm) ◆.** Actions lock at planning, so queuing an interaction with a *clean* unit can still infect it by the time it resolves — and your actor catches it back (Firewall-modified roll; **no same-action re-spread**) (§7).

**Resist & self-cure.** Immunity (Virus) / Firewall (Worm) lowers tick damage and build/spread odds; at **[threshold]** the unit **self-cures**. **Cleanse** removes **[value]** stacks and applies **Vaccinated** (Virus) / **Antimalware** (Worm) — a decaying buff that strips **[value]** stacks/turn and grants temporary resist. **Runaway brake [required]:** ≥1 hard cap per contagion (§8 #9). **On death:** the strain either **spills** (Data-spill → neighbors) or **purges** its stacks.
