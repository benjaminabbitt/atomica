...
> `break_mode`), machine immunity, the ally-death Stress trigger, **Rally / leaders** (the *Anthem*
> projection — a leader regens nearby allies' Resolve each round), and the **leader-death cascade**
> (a leader's fall stresses the formation harder) are all in `crates/sim`. **Still 🔭:** Resolve
> **recovery** (un-breaking), the wider **trigger set** (flanked, heavy hit), and ⏳ tuning.
