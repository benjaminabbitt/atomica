...
**Upgrade normalization — a hidden point budget ◆.** Every upgrade (gear, skill, augment, unlock) carries a **hidden power-point value**, **invisible** to the player (no on-screen number) — a *balancing substrate*, not a currency. **Purpose: balance players with different meta-progression against each other in the async PvP.** Because you fight other players' rosters asynchronously, the point totals normalize matchups (budget caps / handicapping) so a veteran-heavy player and a newer one meet on fair terms — it's not a who-grinded-more contest. The same budget also keeps the survival-earned meta-progression (#17) from snowballing within a run. Factoring: `atomica-content` tags each upgrade with its hidden points; `atomica-run` budgets against them for matchmaking. *(⏳ the values + the matchmaking curve are tuning.)*
...
**Factoring note — engine reconciled (✅), with the morale loop still 🔭.** The stat-line consolidation is now **in the `sim` crate**: `Stat::Firewall → Ice`, the **Health attribute removed** (bio-resist rolls against **Body**: `Resist::Body`, `Contagion.resist = Stat::Body`), **`Corruption::virus` attacks Body** (a `−Body` factor — the wasting that drags Integrity), **Poison** is the pure-DoT softener resisted by Body, and the **`Nerve` attribute** + Resolve-pool derivation (`max_resolve = Nerve × RESOLVE_PER_NERVE`) + `Resist::Nerve` are wired (sim 214 + run 18 green). Two of the three follow-on **features have since shipped too**: **spoof now rolls a Nerve composure contest** (`deploy_rider`: a breach cracks ICE, then Spoof/Misfire must beat `SPOOF_POWER − Nerve` — *ICE guards the system, Nerve guards the self*; a Nerve-0 machine is spoofed freely), and the **active-Guard recast** of `net_defense` is in (the **Sentinel** doctrine spends a runner's activation walling one covered ally via [`Unit::guarding`]; the old free passive `max()` is gone). What's **still unbuilt (🔭)** is the **morale loop** itself — the Resolve pool *in play*, Stress, Break/Rally (§4) — the one genuinely new system left. The docs and code now agree on the stat model, the corruption resists, and the netrunning defense; only the morale layer remains design-only.
