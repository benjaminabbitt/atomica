...
> are all in `crates/sim`, and the **content roster fields it** — the **Anthem** leader (high Nerve,
> `leadership`), a berserk blade, a disciplined bulwark (`crates/run/content.rs`). **Still 🔭:**
> mid-battle Resolve **recovery** (un-breaking a routed unit), and ⏳ tuning the feel-numbers.
