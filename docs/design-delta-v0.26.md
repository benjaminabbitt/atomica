...
> **Status: core built ✅** (§17 factoring note) — the Resolve pool, Stress, Break (rout/berserk by
> `break_mode`), the ally-death Stress trigger, and machine immunity are in `crates/sim`. **Still 🔭:**
> Rally / leaders, recovery, the wider trigger set, and ⏳ tuning.
