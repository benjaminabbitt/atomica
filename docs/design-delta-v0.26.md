...
- **Skills attack, stats defend.** The *only* additive bonus to a roll is **skill** (character or chip — take-the-max, chips capped, §10) **+ equipment** (deck hack-rating, weapon mod, …). **No chassis stat is added to a roll.** Instead the defender's stat line *is* the TN: **Firewall** (hacks / Worm / spoof), **Immunity** (Virus), **Evasion** (physical to-hit — a gun also adds a **range penalty** on top, but **never** to a hack: range can't touch the digital realm), a control-resist — thresholds you roll *against*, never bonuses to your own roll.
