...
9. **[layers.md](layers.md)** — the equipment **decorator** architecture: one `Layer` interface that the base unit and every piece of kit (implants/weapons/armor/corruption) implement, composed over a base. Generalises the cyberware fold; the target to refactor toward before finishing combat Phase 1.
