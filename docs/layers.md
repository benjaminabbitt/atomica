...
|---|---|
| **stats** | `link` · `ice` · `body` · `initiative` · `max_integrity` · `armor_class` |
...
| **L4 ✅** (stat read-through) | the loop reads stats **through the gen**: `Unit` composed accessors `link()`/`ice()`/`body()`/`initiative()`/`max_integrity()` (the bio resist is just `body()` now — no separate `immunity()`); the loop (hack TN/gates, woven order, resist TN, init) calls them. `apply_modifier` installs a stat `Factor` that composes live — an ICE debuff lands a hack, a buff reorders initiative. *(Transitional flat-fields-as-base seam **closed by L5**.)* | `lib.rs` accessors + loop reads + 2 tests |
