...
**built — L1–L7** (see §7/§9); the `Character` gen is the single source of truth —
stats, weapons, armor, behavior and corruption all compose, the flat stat line and the
hand-rolled fold are gone (breach §6, Smartgun/IFF, and the contested **contagion**
spread are all in — `corruption.md`).*
...
|---|---|
| **stats** | `link` · `ice` · `body` · `nerve` · `initiative` · `max_integrity` · `resolve()` (the Resolve pool, `nerve × K`) · `armor_class` |
...
- **Per-stat buckets.** Link, ICE, damage, … each fold independently.
...
generator that adds an `Override` factor on `movement` / `targeting`. A **spoof**
resists off **Nerve** (composure — *Nerve guards the self*), distinct from a worm's
**ICE** resist (*ICE guards the system*).
...
| **L4 ✅** (stat read-through) | the loop reads stats **through the gen**: `Unit` composed accessors `link()`/`ice()`/`body()`/`nerve()`/`initiative()`/`max_integrity()` (the bio resist is just `body()` now — no separate `immunity()`; the spoof / intimidation / Stress resist is `nerve()`, which also sizes the 🔭 PLANNED `resolve()` pool); the loop (hack TN/gates, woven order, resist TN, init) calls them. `apply_modifier` installs a stat `Factor` that composes live — an ICE debuff lands a hack, a buff reorders initiative. *(Transitional flat-fields-as-base seam **closed by L5**.)* | `lib.rs` accessors + loop reads + 2 tests |
...
| **L7 ✅** (remaining threads) | breach §6 closed (Cascade crit-gating, per-effect ladder, the **Worm/logic-bomb** vector, the **knockout-gate knob**, **salvage-on-death**); **Smartgun/IFF** (`Attack.smart` spares the team in the line of fire); and the **contagion** families — a contested **spread** phase (`Battle::contagion_phase`): a corruption carries a `Contagion {virulence, resist, vector}`, jumps along proximity (Virus) / the net (Worm) on a `virulence` contest — bio vs the host's **Body**, digital vs its **ICE** — and copies itself onto the victim (`corruption.md`) | `lib.rs` + `chargen.rs` + `corruption.rs`; 5 tests |
...
✅ **built — L1 through L7.** The `Character` (gen + `BaseLine` + pools) is the single
...
no flat `attack` / `armor_class`). The breach §6 ladder is complete (incl. the
**Worm/logic-bomb** vector, the knockout-gate knob, salvage-on-death), the
**Smartgun/IFF** mod (`Attack.smart`), and the **contagion** families — a contested
spread phase (`corruption.md`). The decorator gen now covers stats, weapons, armor,
behavior, breach, and spreading corruption end to end.
