...
hand-rolled fold are gone (and the breach §6 ladder is complete). **L7+** (Smartgun
targeting, spreading contagion) is next.*
...
| **L7+** (remaining threads) | breach §6 ✅ closed (Cascade crit-gating, per-effect ladder, the **Worm/logic-bomb** vector, the **knockout-gate knob**, **salvage-on-death**). Still open: the **Smartgun/IFF** smart-targeting mod; and the **contagion families** (Virus/Worm **spreading** via Data-spill) — a system of its own | `lib.rs` combat |
...
**Worm/logic-bomb** vector, the knockout-gate knob, and salvage-on-death). **Next
(L7+):** the Smartgun/IFF targeting mod, and the spreading **contagion** families.
